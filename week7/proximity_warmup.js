...
var currentLocation = {lat: 37.818546, long: -144.959482};
var diffArray = [];


for (var i = 0; i < myResults.length; i++) {
  var diffLat = Math.abs(currentLocation.lat - myResults[0].location.lat);
  var diffLong = Math.abs(currentLocation.long - myResults[0].location.long);

  var sumOfSquares = Math.pow(diffLat, 2) + Math.pow(diffLong, 2);
  var distance = Math.sqrt(sumOfSquares);

  diffArray.push({name: myResults[i].name, distance: distance});
